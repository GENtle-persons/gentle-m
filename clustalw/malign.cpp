...
/* negative score means fatal error... exit now!  */
        ...
        for (i=0;i<=nseqs;i++)
          sets[i]=(int*)ckfree((void *)sets[i]);
        sets=(int**)ckfree(sets);
     }
   else
     {
/* clear the memory used for the phylogenetic tree */
...
*/
   ...
   group=(int*)ckfree((void *)group);
   aligned=(int*)ckfree((void *)aligned);
   maxid=(int*)ckfree((void *)maxid);
   tree_weight=(int*)ckfree((void *)tree_weight);
...
/* do the alignment.........  */
   ...
   info("Sequences:%d      Score:%d",(pint)entries,(pint)score);
   group=(int*)ckfree((void *)group);
   p1_weight=(int*)ckfree((void *)p1_weight);
   p2_weight=(int*)ckfree((void *)p2_weight);
   aligned=(int*)ckfree((void *)aligned);
   maxid=(int*)ckfree((void *)maxid);
