...
fprintf(stdout,"\n");
	...
	if (sec_struct_mask != NULL) sec_struct_mask=(char*)ckfree(sec_struct_mask);
	if (gap_penalty_mask != NULL) gap_penalty_mask=(char*)ckfree(gap_penalty_mask);
	if (ss_name != NULL) ss_name=(char*)ckfree(ss_name);
...
*/
	...
	struct_mask=(char*)ckfree(struct_mask);
...
void gcg_out(FILE *gcgout, sint fres, sint len, sint fseq, sint lseq)
{
  ...
  seq=(char*)ckfree((void *)seq);
  all_checks=(int*)ckfree((void *)all_checks);
  fprintf(gcgout,"\n\n");
...
void calc_percidentity(FILE *pfile)
{
  ...
  */  
  ...
  for (i=0;i<nseqs;i++) 
    pmat[i]=(double*)ckfree((void *)pmat[i]);
  pmat=(double**)ckfree((void *)pmat);
