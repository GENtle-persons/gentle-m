...
bool TABIviewer::get_inv_compl () const
    {
...
wxString TABIviewer::getName () const
    {
    return vec->getName() ;
    }

wxString TABIviewer::getStat () const
    {
    SeqABI *abi = (SeqABI*) sc->seq[0] ;
    ABItype *at = abi->at ;
    ...
    char u[256] ;
    wxString bases ;
    for ( int a = 0 ; a < 256 ; a++ ) u[a] = 0 ;
    for ( size_t a = 0 ; a < abi->s.length() ; a++ ) u[abi->s.GetChar(a)]++ ;
    for ( int a = 0 ; a < 256 ; a++ )
       {
       if ( u[a] > 0 )
          {
...
void TABIviewer::showStat () const
    {
    wxString r = getStat () ;
    stat->SetValue ( r ) ;
    }

void TABIviewer::showSequence () const
    {
    // Cleaning up
    CLEAR_DELETE ( sc->seq ) ;
...
void TABIviewer::OnEditName(wxCommandEvent& event)
    {
    wxString nn = wxGetTextFromUser ( txt("t_edit_aa_name_txt") , txt("t_edit_aa_name") , vec->getName() ) ;
