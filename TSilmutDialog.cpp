...
wxString TSilmutDialog::getSequence () const
    {
    int i = selID() ;
    if ( i == -1 ) return _T("") ;
    return vs[i].mut ;
...
TRestrictionEnzyme *TSilmutDialog::getEnzyme () const
    {
    int i = selID() ;
    if ( i == -1 ) return NULL ;
    return vs[i].e ;
...
int TSilmutDialog::selID () const
    {
    if ( last_selection == -1 ) return -1 ;
    for ( int a = 0 ; a < lb_retro.GetCount() ; a++ )
...
void TSilmutDialog::calc ()
    ...
    for ( int a = 0 ; a < re.GetCount() ; a++ )
        ...
        for ( int b = f ; b <= t ; b++ )
            ...
            match = mismatch = 0 ;
            wxString x ;
            for ( int c = 0 ; c < s.length() && mismatch <= limit ; c++ )
                {
            ...
            for ( int c = fragments.GetCount()-1 ; c > 0 ; c-- ) fragments[c] -= fragments[c-1] ;
            fragments[0]++ ;
        ...
        for ( int b = from-1 ; b <= to ; b++ )
           {
            for ( int base = 0 ; base < 4 ; base++ )
                ...
                for ( int c = 0 ; c < vc_new.size() ; c++ ) si.fragments.Add ( vc_new[c].getCut() ) ;
                si.fragments.Add ( v->getSequenceLength()-1 ) ;
                for ( int c = si.fragments.GetCount()-1 ; c > 0 ; c-- )
...
wxString TSilmutDialog::getAAresult ( const wxString& dna )
    {
    if ( !pd )
        {
        wxString s = dna ;
