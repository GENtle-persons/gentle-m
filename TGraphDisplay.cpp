...
void TGraphDisplay::SetZoom ( const int _zx , const int _zy )
    {
...
stringField TGraphDisplay::readTextfile ( const wxString& filename )
    {
...
void TGraphDisplay::setupIPCfile ( const wxString& filename )
    {
...
//  setupPhotometerGraph ( sf ) ;
    ...
    for ( int a = 0 ; a < sf.size() && sf[a].size() > 1 && sf[a][0] != "" ; a++ )
        {
        double x , y ;
...
void TGraphDisplay::setupPhotometerGraph ( const stringField &sf )
    ...
    for ( int a = 2 ; a < sf.size() && sf[a].size() > 1 && sf[a][0] != "" ; a++ )
        {
        double x , y ;
...
void TGraphDisplay::setupFluorimeterGraph ( const stringField &sf )
    ...
    for ( int a = 0 ; a+2 < sf[0].size() ; a += 2 )
        ...
        for ( int b = 2 ; b < sf.size() && ( sf[b].size() > a + 1 && sf[b][0] != "" ) ; b++ )
            {
            double x , y ;
...
bool TGraphDisplay::SetupFromFile ( const wxString& filename )
    {
    wxArrayString as ;
...
unsigned char *TGraphDisplay::readRawData ( const wxString& filename , long &l )
    {
...
void TGraphDisplay::addRawData ( unsigned char *d , long l , const wxString& title )
    {
    stringField sf ;
    TVS b ;
    b.push_back ( "" ) ;
    ...
    long sum = 0 , integrate = 10 , cnt = 0 ;
    for ( long a = 2000+1 ; a+30 < l ; a += 4 )
        {
        unsigned long x = 0 ;
...
void TGraphDisplay::addRawData2 ( unsigned char *d , long l , const wxString& title )
    {
    stringField sf ;
    TVS b ;
    b.push_back ( "" ) ;
...
void TGraphDisplay::addNewGraph ( const stringField &sf , const wxString& title , TGraphScale *sx , TGraphScale*sy , int startrow )
    {
    TGraphData *ng = new TGraphData ( this ) ;
    ng->name = title ;
    ng->SetScales ( sx , sy ) ;
...
void TGraphDisplay::setupRawFPLC ( const wxString& filenamebase )
    {
     TGraphScale *sx = new TGraphScale ( 0 , 0 , true , false , _T("X") , _T("") , *wxBLACK ) ;
     TGraphScale *sy = new TGraphScale ( 0 , 0 , false , true , _T("Y") , _T("") , *wxBLACK ) ;
     scales.push_back ( sx ) ;
...
wxString TGraphDisplay::tryall ( const wxString& filename )
    {
    wxString r ;
    int best = 0 ;
    int a , cnt ;
...
void TGraphDisplay::setupBioFormat ( const wxString& filenamebase )
    {
 ...
 * Not used
 */
void TGraphDisplay::setupDUF ( const wxString& filenamebase )
    {
...
void TGraphDisplay::drawit ( wxDC &dc , const int mode ) /* not const */
    {
    dc.Clear () ;
...
void TGraphDisplay::OnEvent(wxMouseEvent& event)
    ...
    // Over a scale?
    for ( int a = 0 ; !event.Dragging() && a < scales.size() ; a++ )
        {
    ...
    // Dragging (box)?
    else if ( inner.Contains ( pt ) && event.ControlDown() && event.Dragging() && !event.RightIsDown() && !event.MiddleIsDown() )
        {
        ...
        if ( draggingRect.x == -1 )
            ...
            if ( event.ShiftDown() )
                {
                if ( draggingRect.width > draggingRect.height ) draggingRect.height = draggingRect.width ;
...
bool TGraphDisplay::IsSetupComplete() const
    {
