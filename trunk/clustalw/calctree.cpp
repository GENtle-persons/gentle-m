...
void clear_tree(treeptr p)
{
   ...
   nptr=(node**)ckfree((void *)nptr);
   ptrs=(node**)ckfree((void *)ptrs);
   lptr=(node**)ckfree((void *)lptr);
   olptr=(node**)ckfree((void *)olptr);
}
...
*/
	...
	dist2node=(float*)ckfree((void *)dist2node);
	path2root=(node**)ckfree((void *)path2root);
...
static void group_seqs(treeptr p, sint *next_groups, sint nseqs)
{
    ...
    tmp_groups=(int*)ckfree((void *)tmp_groups);
...
*/
   ...
   for (i=0;i<nseqs;i++) dmat[i]=(double*)ckfree((void *)dmat[i]);
   dmat=(double**)ckfree((void *)dmat);
