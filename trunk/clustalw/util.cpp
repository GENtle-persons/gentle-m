...
void free_aln(sint nseqs)
{
	...
	seqlen_array = (int*)ckfree(seqlen_array);

        for(i=1;i<=nseqs;i++)
		seq_array[i] = (char*)ckfree(seq_array[i]);
	seq_array = (char**)ckfree(seq_array);

	for(i=1;i<=nseqs;i++)
		names[i] = (char*)ckfree(names[i]);
	names = (char**)ckfree(names);

	for(i=1;i<=nseqs;i++)
		titles[i] = (char*)ckfree(titles[i]);
	titles = (char**)ckfree(titles);

	output_index = (int*)ckfree(output_index);

	seq_weight = (int*)ckfree(seq_weight);

        for(i=1;i<=nseqs;i++)
		tmat[i] = (double*)ckfree(tmat[i]);
	tmat = (double**)ckfree(tmat);
}
...
{
	seq_array[seq_no]=(char*)ckfree(seq_array[seq_no]);
}
