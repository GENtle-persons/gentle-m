...
*/
	tree_gaps=(char*)ckfree((void *)tree_gaps);
	boot_positions=(int*)ckfree((void *)boot_positions);
	if (left_branch != NULL) left_branch=(double*)ckfree((void *)left_branch);
	if (right_branch != NULL) right_branch=(double*)ckfree((void *)right_branch);
	if (tkill != NULL) tkill=(int*)ckfree((void *)tkill);
	if (av != NULL) av=(double*)ckfree((void *)av);
	for (i=0;i<nseqs+1;i++)
		standard_tree[i]=(char*)ckfree((void *)standard_tree[i]);
	standard_tree=(char**)ckfree((void *)standard_tree);

	for (i=0;i<nseqs+1;i++)
		save_tree[i]=(char*)ckfree((void *)save_tree[i]);
	save_tree=(char**)ckfree((void *)save_tree);
...
*/
	...
	left_branch=(double*)ckfree((void *)left_branch);
	right_branch=(double*)ckfree((void *)right_branch);
	tkill=(int*)ckfree((void *)tkill);
	av=(double*)ckfree((void *)av);
...
/* check if any distances overflowed the distance corrections */
	...
	for (i=1;i<nseqs+1;i++)
		sample_tree[i]=(char*)ckfree((void *)sample_tree[i]);
	sample_tree=(char**)ckfree((void *)sample_tree);
/*
...
*/
	...
	boot_totals=(int*)ckfree((void *)boot_totals);
	save_left_branch=(double*)ckfree((void *)save_left_branch);
	save_right_branch=(double*)ckfree((void *)save_right_branch);
