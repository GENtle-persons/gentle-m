...
*/
     ...
     aln_weight=(int*)ckfree((void *)aln_weight);
...
/* use Myers and Miller to align two sequences */
  ...
  HH=(int*)ckfree((void *)HH);
  DD=(int*)ckfree((void *)DD);
  RR=(int*)ckfree((void *)RR);
  SS=(int*)ckfree((void *)SS);
  gS=(int*)ckfree((void *)gS);

  ptracepath( &alignment_len);
  
  displ=(int*)ckfree((void *)displ);

  add_ggaps();

  for (i=0;i<prf_length1+2;i++)
     profile1[i]=(int*)ckfree((void *)profile1[i]);
  profile1=(int**)ckfree((void *)profile1);

  for (i=0;i<prf_length2+2;i++)
     profile2[i]=(int*)ckfree((void *)profile2[i]);
  profile2=(int**)ckfree((void *)profile2);

  prf_length1 = alignment_len;

  aln_path1=(char*)ckfree((void *)aln_path1);
  aln_path2=(char*)ckfree((void *)aln_path2);
  ...
  for (i=0;i<nseqs1+nseqs2;i++)
     alignment[i]=(char*)ckfree((void *)alignment[i]);
  alignment=(char**)ckfree((void *)alignment);

  aln_len=(int*)ckfree((void *)aln_len);
  gaps=(int*)ckfree((void *)gaps);
