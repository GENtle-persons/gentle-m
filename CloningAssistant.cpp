...
void TCloningAssistant::Refresh (const bool eraseBackground , const wxRect* rect )
    {
    panel->Refresh () ;
    }
...
TDDR *TCloningAssistant::new_from_vector ( TVector * const v , const int drag ) const
    {
...
void TCloningAssistantPanel::arrange ()
    ...
    for ( int a = 0 ; a < ca->vlist->children.size() ; a++ )
        {
