...
bool TManageDatabaseDialog::copyDNA ( const wxString& _n , const wxString& sdb , const wxString& tdb )
    {
    ...
    // Already there?
    wxString sql = "SELECT dna_name FROM dna WHERE dna_name=\"" + name + "\"" ;
    TSQLresult r = target->getObject ( sql ) ;
    if ( r.rows() > 0 )
        {
