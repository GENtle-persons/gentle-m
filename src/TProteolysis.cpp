...
void TProteolysisGel::OnEvent(wxMouseEvent& event)
    ...
    wxArrayInt res ;
    int best = 99999 ;
    for ( int a = 0 ; a < screen.GetCount() ; a++ )
        {
...
void TProteolysis::calc_spearation_sub ( int depth , TProteaseArray &prop , vector <TFragment> &tobe , int start )
    ...
    int cur = prop.size() ;
    prop.push_back ( NULL ) ;
    for ( int a = start ; a < cutting_proteases.size() ; a++ )
        {
        ...
        for ( int b = 1 ; b < tobe.size() ; b++ )
            {
            bool sep = false ;
            for ( int c = 0 ; !sep && c < apc.size() ; c++ )
                {
...
void TProteolysis::calc_cut_list ()
    ...
    for ( int a = 0 ; a+1 < pc.size() ; a++ )
        {
        wxString s = wxString::Format ( txt("t_proteolysis_cut_after") , pc[a]->cut , pc[a]->protease->name.c_str() ) ;
        cuts->Append ( s ) ;
        cuts->Check ( a ) ;
        }
    }
...
void TProteolysis::show_gel () const
    {
    wxClientDC dc ( gel ) ;
    draw_gel ( dc ) ;
    }

void TProteolysis::draw_gel ( wxDC &dc ) const
    {
    wxString s ;
    int y , tw , th , w , h ;
    dc.GetSize ( &w , &h ) ;
...
int TProteolysis::get_y ( const double y , const int _h , const double min , const double max ) const
    {
    int h = _h - 20 ;
...
double TProteolysis::get_weight ( const int from , const int to ) const
    {
    double w = 0 ;
    ...
    if ( create_labels->IsChecked() )
        ...
        for ( int a = 0 ; a < fragments.size() ; a++ )
            ...
            name += wxString::Format ( _T(" (%d-%d; %dbp; %2.2fkD)") ,
                                       ...
                                       fragments[a].length ,
                                       fragments[a].weight ) ;
            TVectorItem i ( name , desc , fragments[a].from , fragments[a].to , VIT_MISC ) ;
...
void TProteolysis::OnAll ( wxCommandEvent &ev )
    {
    for ( int a = 0 ; a < results->GetCount() ; a++ )
        results->Check ( a , true ) ;
    show_gel() ;
    }
...
void TProteolysis::OnSortResults ( wxCommandEvent &ev )
    {
    int mode = sortresults->GetSelection() ;
...
void TProteolysis::OnSepResults ( wxCommandEvent &ev )
    {
    int i = sep_results->GetSelection () ;
    ...
    for ( int a = 0 ; a < cutting_proteases.size() ; a++ )
        ...
        bool check = false ;
        for ( int b = 0 ; !check && b < suggestions[i].proteases.size() ; b++ )
            {
...
void TProteolysis::OnReport ( wxCommandEvent &ev )
{
    ...
    // Protease list
    wxString html = txt("t_vec_prot") + _T(" : ") ;
    int nop = proteases->GetCount() ;
    for ( int a = 0 ; a < nop ; a++ )
        {
        if ( !proteases->IsChecked ( a ) ) continue ;
